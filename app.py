...
# ==========================================
# STEP 4: RULES ENGINE DASHBOARD (Header Visible, Content Locked)
# ==========================================
st.subheader("⚙️ STEP 4: Configure Computing Engine")

if st.session_state.merged_df is None:
    st.info("🔒 Please complete the VLOOKUP merge in **STEP 3** to unlock the Rules Engine Configuration.")
else:
...
if st.session_state.merged_df is None:
     st.info("🔒 Ensure you have uploaded data and configured your rules above to unlock the Final Execution Engine.")
else:
    ...
    if st.button("⚡ APPROVE LOGICS & CALCULATE FINAL OUTPUT", use_container_width=True, type="primary"):
        with st.spinner("Engaging Computing Engine..."):
            ...
            base_cols = [col for col in st.session_state.available_cols]
